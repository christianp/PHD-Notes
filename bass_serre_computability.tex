...
\begin{lemma} \cite[2.7]{Rose_1984}
...
\begin{lemma} \cite[2.9]{Rose_1984}
...
\ajd{(The length of $[x_0,\ldots, x_{n-1}]$ is then $n$.) To reassure the reader.}
...
\ajd{There should be a definition of \grz{n}-computable function and \grz{n}-decidable
set and what it means for a function $i: V \rightarrow \ZZ_{\geq 0}$ to be 
\grz{n}-decidable, where $V$ is an arbitrary set,  
before this definition.(The latter means that $i$ is injective and its image is \grz{n}-decidable,
 I believe.}
\begin{definition}
...
Note that he depth-first walk of the subtree descended from a vertex $v_n$ is the subsequence found between the two occurrences of $n$ in $\tvec$. 
...
Cannonito \cite{Cannonito_1973} introduced the {\it relativised Grzegorczyk hierarchy} relative to a set $A$, which includes the characteristic function of $A$ as a primitive function.
