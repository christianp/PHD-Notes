...
In section \ref{groups} we restate \cite{Cannonito_1966}'s definition of an \grz{n}-computable group and \grz{3}-computable index of the free group on countably many generators. We also discuss a flaw in \cite{Cannonito_1973}'s concept of a ``standard'' group.
...
A \textit{bound variable} is one which is used to define a function, not passed as a parameter into it. For instance, in the porjection function $P_n^i$, $n$ and $i$ are bound variables. Bound variables do not contribute to the arity of a function.
...
If $R \subseteq \NN^d$ is a relation, $R$ is defined by $\chi_R$. In addition, for binary relations I will write
\[ x R y := \chi_R(x,y). \]
...
A class \classC{} is {\it closed under bounded recursion} if, whenever $f,g,h$ belong to \classC{} and $R(f,g)(\xvec)$ is bounded by $h(\xvec)$, then $R(f,g)$ also belongs to \classC{}. 
...
Bounded minimisation: given a predicate $P(\xvec,i)$ and a bound $y$ this returns the least $i$ strictly less than $y$ such that $P(\xvec,i)$ is true. If there is no such $i$, $y$ is returned.
...
\recur{
	...
	P_f(\xvec,y+1) \equiv \prod_{i < y+1} f(\xvec,i) &:= f(\xvec,y+1) \cdot \prod_{i < y} f(\xvec, i).
}
...
The $+1$ in the exponent is so we can reliably determine the length of a list. 
\ajd{The empty list should be encoded as $1$: then it will have length $0$.} 
...
A {\it rooted} tree has a particular vertex $v_0$ said to be at the 'top'. The children of a vertex $v$ are those vertices adjacent to $v$ and further from the root than $v$. In an {\it ordered} tree, the set of children of each vertex is ordered, so there is a least, or leftmost, child, and so on. 
\end{definition}
...
We can assign a labelling to an ordered recursive tree by giving vertices labels corresponding to the order in which they were added to the tree, beginning with $0$ for the root. This way, the sequence of labels encountered on a path heading downwards is always increasing. 
...
\begin{itemize}
	\item $i: G \rightarrow \NN$ is an injection of $G$ onto an \grz{n}-decidable subset of $\NN$;
...
Let $P=(P,D,m,\eps,~^{-1})$ be a pregroup and let $i$ be an injective function $i:P\maps \NN$.
...
\item 
the function 
$m^\prime:D^\prime \maps \NN$, such that $m^\prime(i(p),i(q))=i(m(p,q))$, 
for 
all $(p,q)\in D$,  
and 
...
as appropriate to
the context. 
Let $i:P\maps \NN$ be an injective function and assume that $i(P)$ is \grz{n}-decidable.
...
operation on $P$ to $P^*$. That is we define $m_1:\NN\times\NN\maps \NN$ and $j_1:\NN\maps \NN$ by
...
Next define the predicate $\Preduced$ on $\NN$ by
...
Define a function $\leftm:\NN\maps \NN$ by
...
Now define $\Predn:\NN\times \NN \maps \NN$ by
\begin{align}
\Predn(\avec,0) & = \avec,\notag\\
...
Finally define $\Preduce:\NN\maps \NN$ by
...
$\approx$ equivalence class we define the relation $\Interleaven\subseteq \NN\times \NN$ 
defined as 
...
We define a function $i_2:\NN\maps \NN$ which will be used to construct an
index for $\UP$. 
\begin{equation}\label{eq:i2}
i_2(\avec)=\min_y(y<\avec \wedge y\in i_1(P) \wedge \Preduced(y) \wedge y\approx \avec).
\end{equation}
Now define functions $i_G:P^*\maps \NN$, 
$m_G:i_G(P^*)\times i_G(P^*) \maps \NN$
and $j_G:i_G(P^*)\maps \NN$ by 
\begin{align}
...
$P$-reduced words and let $S$ be the image $i_G(R)$ of $R$ under $i_G$. Then, for $\xvec\in \NN$, 
