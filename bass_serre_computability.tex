...
\ajd{In the lists of functions below some definitions start with ``The'' and 
others don't. (e.g. ``The smallest of two numbers'' and ``Remainder''). I shall add ``The''
where it's appropriate, unless you object for some reason. Similarly, there are ``I''s 
and ``we''s scattered randomly. I shall change all ``I'' to ``we'' if you don't stop me.}
\subsection{Notation}
Expressions consist of operators, function names, variables and punctuation. An expression can only be evaluated to a single numerical value if it does not contain any variables.
\ajd{It's still not clear to me what you're getting at here. 
The function $z_1$ given by $z_1(0)=z()$ and $z_1(n+1)=P_2^2(n,z_1(n))$, takes the value $0$ 
at every point $x\in\ZZ_{\geq 0}$, but could be written as the 
expression $z_1(x)=0$, which contains a variable.
(On the other hand there
are expressions like $.(f-+.($ that contain no variables and have
no evaluation.)}
...
\begin{itemize}
	\item {\em The zero function}. The zero function \ajd{$z$} is a nullary function which returns the number $0$. For brevity I will write the digit $0$ to mean the zero function, instead of using the function notation with brackets defined above.
...
Substitution of expressions into expressions with free variables can also be dealt with systematically, by composing with the projection function $P_i^i$ everywhere except at the position which will take the substituted expression. In the expression $f(x_1, \dots, x_n)$, the $x_i$ are called {\it free variables} because they can take any value. Any free variable can be replaced with another expression $y$, defined in terms of the free variables, like so:
\[f(x_1, \dots, y, \dots, x_n) := f(x_1, \dots, y(x_1, \dots, x_n), \dots, x_n) = C(f,P_n^1, \dots, y, \dots, P_n^n). \]
\ajd{What exactly does ``can take any value'' mean? What exactly
 is an ``expression defined in terms of free variables''? 
Normally ``bound variables'' are
defined as those variables $x$ which occur in a term of the form $Qx$,
where $Q$ is a quantifier ($\exists$ or $\forall$) and then free variables
are those which are not bound. There are technicalities to overcome, because
a variable can be bound in one formula of an expression and not in another,
so your definition maybe sensible, but perhaps you should point out 
that there are more formal versions (and refer to somewhere where
they can be found).  
I'm guessing what you mean here is that $y$ can be substituted for
 a free variable $x$ in an expression, if the variables of $y$ are 
all free.
} 
...
\begin{lemma} \cite[\ajd{Theorem/Lemma} 2.5]{Rose_1984}
	Every function $f$ in \grz{0} satisfies
	\[ f(x_1, \dots, x_n) \leq x_i + k, \]
	for some integers $k$ and $i$ where $1 \leq i \leq n$.
\end{lemma}

\begin{lemma} \cite[\ajd{Theorem/Lemma} 2.6]{Rose_1984}
	...
	That is, every function in \grz{1} is bounded by a linear combination of its arguments.
\end{lemma}

\begin{lemma} \cite[\ajd{Theorem/Lemma} 2.7]{Rose_1984}
...
\begin{lemma} \cite[\ajd{Theorem/Lemma} 2.9]{Rose_1984}
...
\ajd{(Thus the length of $[x_0,\ldots, x_{n-1}]$ is $n$.) To reassure the reader.}
...
\ajd{Every edge $e_{ij}$ can be given a unique label $\max(i,j)$ (irrespective of choice
of pos and neg).  If positive edges are defined to be the ones pointing downwards (away
from the root) then the label of a positive edge $e_{i,j}$ is always $j$. Positive and 
negative edges $e$ and $\bar e$ have the same label.}
If we say that the positive edges point downwards, every positive edge can also be given a unique labelling by saying that the edge $e_{ij}$ has label $max(i,j)$. Because it makes easier the computation I will describe shortly, each negative edge will have the same label as its positive counterpart.

\ajd{Somewhere before this definition, there should be a definition of \grz{n}-computable function 
(from $S\subseteq \ZZ_{\geq 0}$ to $\ZZ_{\geq 0}$)  and \grz{n}-decidable
set. Also, either a statement of what it means for a function 
$i: V \rightarrow \ZZ_{\geq 0}$ to be 
\grz{n}-decidable, where $V$ is an arbitrary set, is needed
(it means that $i$ is injective and its image is \grz{n}-decidable) or the definition
of \grz{n}-computable tree should change to be more like the definition of 
\grz{n}-computable group. Being computable on a subset $S$ is the same as being partially computable
with domain $S$, where $S$ is decidable. Such functions can be extended to (total) computable
functions, by assigning an arbitrary constant value at each point not in $S$.}
\begin{definition}
...
\ajd{Should this be
\begin{equation*} \operatorname{haschild}(\tvec,n) :=  \min_{i < \tvec}(\operatorname{child}(\tvec,i)=n) < \operatorname{kids}(\tvec)? \end{equation*}
}
...
\ajd{The function $z$ is $z(\tvec, n)$, so maybe the definitions should say
$z=z(\tvec,n)=...$. Also, $z$ is already a function so maybe $d$ for descendant would be better.}
...
\begin{lemma}\cite[Lemma 3.1]{Cannonito_1973}
...
which is what Cannonito and Gatterdam must mean. }
$J(x,y)$ is an \grz{3}-computable integer pairing function $J:\ZZ_{\geq 0} \times \ZZ \rightarrow \ZZ_{\geq 0}$ defined 
\ajd{ by $J(x,y)=((x+y)^2+x)^2+y$ (see %in
 \cite{Cannonito_1973} for more details)}.
...
\begin{definition} \cite[Definition 2.3]{Cannonito_1973}
	...
	\end{align}
\end{definition}
 \ajd{$\chi_A$ has already been defined as the characteristic function of $A$, so should 
be used instead of $c_A$. The way it's defined above $\chi_A=\rsg(c_A)$, but probably 
it would be best to change this.}
...
\begin{corollary} \label{wp-iff-group}
	When $n \geq 3$ \ajd{and $G$ is a countable group}, WP$(G)$ is \grz{n}-decidable if and only if $G$ is \grz{n}-computable. Also, WP$(G)$ is not \grz{n}-decidable if and only if $G$ is not \grz{n}-computable.
 \ajd{The second statement is equivalent to the first. However there is something wrong with
both. $WP(G)$ is not a subset of $\ZZ_{\geq 0}$. To make sense of the statement an
index $(i,m,j)$ for the free group $F(X)$ is needed, and then $A_i=i(WP(G))$ will be a subset
of $i(F(X))\subseteq \ZZ_{\geq 0}$. If $A_i$ is \grz{n}-decidable, then $G$ is \grz{n}-computable,
as shown in \cite[Theorem 3.2]{Cannonito_1973}. Is the converse true?}
\end{corollary}
