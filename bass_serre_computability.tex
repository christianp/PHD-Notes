...
\newcommand{\avec}{\mathbf{a}}	% vector x
\newcommand{\bvec}{\mathbf{b}}	% vector x
\newcommand{\cvec}{\mathbf{c}}	% vector x
\newcommand{\dvec}{\mathbf{d}}	% vector x
...
\begin{equation} \operatorname{find}(\xvec,y) := \min_{i < |\xvec|} (\xvec)_i = y. \end{equation}
Note that $\operatorname{find}(\xvec,y)$ returns $|\xvec|$ if $y$ does not occur in the list.
...
\begin{definition}
A pregroup $(P,D)$ is \grz{n}$(A)$-\emph{computable}, with respect to
