...
\[ \lfloor x^{\frac{1}{2}} \rfloor := \min_{n < x}{\left ( n \cdot n < x \right )} \]
...
$J(x,y)$ is an \grz{3}-computable integer pairing function $J:\NN \times \ZZ \rightarrow \NN$ defined in \cite{Cannonito_1973}.
...
\begin{corollary}
	WP$(G)$ is \grz{n}-decidable if and only if $G$ is \grz{n}-computable.
\end{corollary}
...
What we really want is a function $\pi:G \rightarrow X^{\ast}$ that gives an admissible word beginning and ending at $v_0$ and containing a representation of $g$ as a contiguous subword.  So $\pi(g)$ is an admissible word which represents $g \in G$.
