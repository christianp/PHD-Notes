...
Expressions consist of operators, function names, variables and punctuation. An expression can only be evaluated if it does not contain any variables.
\ajd{Reverse the order of the 2 previous paragraphs (Define expression and
then talk about notation). What does ``evaluated''
mean? Do you mean for example that the function $f$ can be evaluated at
the point $x=n$, but that $f(x)$ cannot be evaluated at the point $x=n$?}
...
\[ x R y := \chi_R(x,y). \]
\ajd{This is really a definition: ``A $d$\emph{-ary relation on} $\ZZ_{\geq 0}$ is
...
\begin{definition} \cite{Rose_1984}
	Paraphrasing Rose, each class \grz{n+1} of the Grzegorczyk hierarchy contains all of \grz{n} plus the function $E_n$, closed under composition and bounded recursion.
\end{definition}
\ajd{Do you  mean that, for each $n\ge 0$, \grz{n+1} is the 
smallest class which 
 contains \grz{n} plus the function $E_n$, and is closed under composition and bounded recursion?}
