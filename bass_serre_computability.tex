...
\subsection{Notation}
Expressions consist of operators, function names, variables and punctuation. \ajd{It's still not clear to me what you're getting at here. 
...
no evaluation.)}
\cp{Then I'm fine with leaving it out entirely. I think I had deleted it before, but it reappeared during a git merge.}
...
\begin{itemize}
	\item {\em The zero function}. The zero function \ajd{$z$}\cp{disagree. Its name is 0 and 0 is its only name. Why overload the meaning of the letter $z$?} is a nullary function which returns the number $0$. For brevity I will write the digit $0$ to mean the zero function, instead of using the function notation with brackets defined above.
...
\cp{I've got rid of the `free variables' thing.

The motivating example for this bit was scenarios like this:

Suppose we have $f(x_1,x_2) \equiv x_1 + x_2 := R(P_1^1,C(s,P_3^2))$. We can construct a function which computes $2x_1+x_2 = x_1+(x_1+x_2)$ by substituting $x_1+x_2$ for $x_2$ in the definition of $f$. That's done by the composition $g := C(f,P_2^1,f)$. 
When evaluating $g(1,2)$, we first work out $P_2^1(1,2) = 1$, and $f(1,2) = 3$. So $g(1,2) = f(1,f(1,2)) = f(1,3) = 4$.

So I was just clearing up that if you have two $n$-ary functions $f(x_1,\dots,x_n)$ and $g(x_1,\dots,x_n)$, the way you construct $f(x_1,\dots,x_{i-1},g,x_{i+1},\dots,x_n)$ is by doing the composition $C(f,P_n^1,\dots,P_n^{i-1},g,P_n^{i+1},\dots,P_n^n)$.

All the talk of ``free variables'' was because I wanted to talk in the context of the friendly notation rather than the non-human-readable language of primitive functions and $C$ and $R$.
}
...
Every edge $e_{ij}$ can be given a unique label $\max(i,j)$ (irrespective of choice of pos and neg).  If positive edges are defined to be the ones pointing downwards (away from the root) then the label of a positive edge $e_{i,j}$ is always $j$. Positive and negative edges $e$ and $\bar e$ have the same label.
