...
Grzegorczyk \cite{Grzegorczyk_1953} introduced a hierarchy of classes of recursive functions delineated by the number of times unbounded recursion is used in the construction of their members. Cannonito \cite{Cannonito_1966} defined a notion of computability for groups in the context of this hierarchy. Cannonito and Gatterdam \cite{Cannonito_1973} \cite{Gatterdam_1973} showed that taking the free product with amalgamation or an HNN extension of group(s) at a particular level of the hierarchy creates a group which is computable at most one level higher.
...
The main result of this paper is that the fundamental group of a graph of \grz{n} groups is \grz{n+1}-computable, generalising the results of \cite{Cannonito_1973}. We also define computability for Stallings' pregroups \cite{Stallings_1971} in the context of the Grzegorczyk hierarchy.
...
\section{Computability of Bass-Serre groups}
...
(this looks horrible, please help!)
\end{definition}
...
So the word problem of $G$ is \grz{n+1}(A) decidable, and hence $G$ is \grz{n+1}-computable.
\end{proof}
